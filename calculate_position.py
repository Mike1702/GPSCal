...
def main():
    ...
    gnss = pd.read_csv('data/%s/device_gnss_sat.csv' % path, dtype={'SignalType': str})
    ...
    for i, (t_utc, df) in enumerate(tqdm(gnss.groupby('utcTimeMillis'), total=nepoch)):
        ...
        # pr = (df_pr['RawPseudorangeMeters'] + df_pr['SvClockBiasMeters_x']).to_numpy()
        # prr = (df_prr['PseudorangeRateMetersPerSecond'] +
        #        df_prr['SvClockDriftMetersPerSecond_x']).to_numpy()

        # # Satellite position/velocity
        # xsat_pr = df_pr[['SvPositionXEcefMeters_x', 'SvPositionYEcefMeters_x',
        #                  'SvPositionZEcefMeters_x']].to_numpy()
        # xsat_prr = df_prr[['SvPositionXEcefMeters_x', 'SvPositionYEcefMeters_x',
        #                    'SvPositionZEcefMeters_x']].to_numpy()
        # vsat = df_prr[['SvVelocityXEcefMetersPerSecond_x', 'SvVelocityYEcefMetersPerSecond_x',
        #                'SvVelocityZEcefMetersPerSecond_x']].to_numpy()

        pr = (df_pr['RawPseudorangeMeters'] + df_pr['SvClockBiasMeters_y']).to_numpy()
        prr = (df_prr['PseudorangeRateMetersPerSecond'] +
               df_prr['SvClockDriftMetersPerSecond_y']).to_numpy()

        # Satellite position/velocity
        xsat_pr = df_pr[['SvPositionXEcefMeters_y', 'SvPositionYEcefMeters_y',
                         'SvPositionZEcefMeters_y']].to_numpy()
        xsat_prr = df_prr[['SvPositionXEcefMeters_y', 'SvPositionYEcefMeters_y',
                           'SvPositionZEcefMeters_y']].to_numpy()
        vsat = df_prr[['SvVelocityXEcefMetersPerSecond_y', 'SvVelocityYEcefMetersPerSecond_y',
                       'SvVelocityZEcefMetersPerSecond_y']].to_numpy()
